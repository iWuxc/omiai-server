...
func (c *Controller) ClientTrend(ctx *gin.Context) {
	var days int
	...
	trend, err := c.client.GetClientTrend(ctx, days)
	if err != nil {
		response.ErrorResponse(ctx, response.DBSelectCommonError, "获取客户趋势失败")
		return
	}

	response.SuccessResponse(ctx, "ok", trend)
}

func (c *Controller) MatchTrend(ctx *gin.Context) {
	var days int
	if d := ctx.Query("days"); d != "" {
		if _, err := fmt.Sscanf(d, "%d", &days); err != nil {
			days = 30
	...
	trend, err := c.match.GetMatchTrend(ctx, days)
	if err != nil {
		response.ErrorResponse(ctx, response.DBSelectCommonError, "获取撮合趋势失败")
		return
	}
