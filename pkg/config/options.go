...
func defaultDecoder(src *KeyValue, target map[string]interface{}) error {
	...
	// Format is not empty, then decode config from src.Value to target.
	// format eg: json, yaml, toml...

	c := viper.New()
	c.SetConfigType(src.Format)
	if err := c.ReadConfig(bytes.NewBuffer(src.Value)); err != nil {
		return errors.Wrap(err, "config decode error")
	}
