...
// MiddlewareFunc is a function which receives a queue.Handler and returns another Handler.
...
func (mux *ServeMux) Handle(pattern string, handler Handler) {
	...
	if strings.TrimSpace(pattern) == "" {
		panic("go-kit: invalid pattern")
	}
	if handler == nil {
		panic("go-kit: nil handler")
	}
	if _, exist := mux.m[pattern]; exist {
		panic("go-kit: multiple registrations for " + pattern)
	}
...
func appendSorted(es []muxEntry, e muxEntry) []muxEntry {
	...
	// we now know that I point at where we want to insert.
...
func (mux *ServeMux) Use(fns ...MiddlewareFunc) {
	mux.mu.Lock()
	defer mux.mu.Unlock()
	//mux.mws = append(mux.mws, fns...)
	for _, fn := range fns {
...
func NotFound(ctx context.Context, task *Task) error {
	return errors.Errorf("handler not found for task %q", task.Type())
}
